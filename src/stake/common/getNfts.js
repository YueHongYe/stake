...
export async function getNFTsByOwner(owner, conn) {
    ...
    nfts.map((nft, index) => {
        ...
        console.log("nft creators:",nft.onchainMetadata.data.creators[0].address)
        console.log("env creator id:",env.creator_id)
        if(nft.onchainMetadata.data.creators[0].address === env.creator_id){
            returnNfts.push(nft)
        }else{
            console.log("beiguolv -----------------")
        }
    })
    return returnNfts;
}
